...
Window.loop do
    ...
    when 2
        Window.draw_scale(19, 374, window_img, 0.5, 0.5, 0, 0)
        Window.draw_font(44, 404, "学校中が賑わって、\nあっちもこっちも楽しそうな日だ！", font, color:[165, 83, 126])
        if Input.mouse_push?(M_LBUTTON)
            flag2 = 3
        end
    when 3
...
　　when 7
	...
	　case flag2
            ...
            Window.draw(125, 0, normal_img)
            Window.draw_scale(19, 374, window_img, 0.5, 0.5, 0, 0)
            Window.draw_font(44, 404, "あれとこれと、あとそれ美味しかった！", font, color:[165, 83, 126])
...
　　　　　　when 9
        ...
        　　Window.draw_scale(19, 374, window_img, 0.5, 0.5, 0, 0)
        　　Window.draw_font(110, 374, "中野黄治", font_name, color:[255, 255, 255])
       　　 Window.draw_font(44, 404, "僕たち、食べ物の好み同じなんだね。", font, color:[165, 83, 126])
            Window.draw_font(0, 0, "好感度：#{likability}", font, color:[255, 255, 255])
       　　 #if Input.mouse_push?(M_LBUTTON)
	 end
	 else
...
　　　　　　when 14
       ...
       　　 #if Input.mouse_push?(M_LBUTTON)
        ...
        if flag == 1 && flag_end == 0 then
            ...
            Window.draw_font(239, 229, "食べ物のお店、全制覇しよう", font, color:[165, 83, 126])
            # 選択肢2
            ...
            Window.draw_font(269, 329, "ミスターコンテスト見に行こう", font, color:[165, 83, 126])

            if timer >= 45
                # マウスの座標が特定の領域にある間、選択肢をホバー状態にする
                if (109 < x && x <541 && 224 < y && y < 260) then # 選択肢1
                    Window.draw_scale(109, 224, button_hover_img, 0.6, 0.6, 0, 0)
                    Window.draw_font(239, 229, "食べ物のお店、全制覇しよう", font, color:[255, 255, 255])
                    if Input.mouse_push?(M_LBUTTON) then
                        player_chose = 1
                        flag = 0
                        flag_end = 1
                    ...
                    Window.draw_font(269, 329, "ミスターコンテスト見に行こう", font, color:[255, 255, 255])
                    if Input.mouse_push?(M_LBUTTON) then
                        player_chose = 3
                        flag = 0
                        flag_end = 1
