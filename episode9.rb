...
　　when 11
        ...
        if flag == 1 && flag_end == 0 then
            ...
            Window.draw_font(239, 229, "偶然だね、私も中野くんにプレゼント用意してたの。\n手袋だよ。", font, color:[165, 83, 126])
            # 選択肢2
            Window.draw_scale(109, 274, button_img, 0.6, 0.6, 0, 0)
            Window.draw_font(259, 279, "可愛いね。気に入ったよ。", font, color:[165, 83, 126])
            # 選択肢3
            Window.draw_scale(109, 324, button_img, 0.6, 0.6, 0, 0)
            Window.draw_font(269, 329, "もっと豪華なのが良かったなぁ…", font, color:[165, 83, 126])

            if timer >= 45
                # マウスの座標が特定の領域にある間、選択肢をホバー状態にする
                if (109 < x && x <541 && 224 < y && y < 260) then # 選択肢1
                    Window.draw_scale(109, 224, button_hover_img, 0.6, 0.6, 0, 0)
                    Window.draw_font(239, 229, "偶然だね、私も中野くんにプレゼント用意してたの。\n手袋だよ。", font, color:[255, 255, 255])
                    if Input.mouse_push?(M_LBUTTON) then
                        player_chose = 1
                        flag = 0
                        flag_end = 1
                    ...
                    Window.draw_font(259, 279, "可愛いね。気に入ったよ。", font, color:[255, 255, 255])
                    if Input.mouse_push?(M_LBUTTON) then
                        player_chose = 2
                        flag = 0
                        flag_end = 1
                    ...
                    Window.draw_font(269, 329, "もっと豪華なのが良かったなぁ…", font, color:[255, 255, 255])
                    if Input.mouse_push?(M_LBUTTON) then
                        player_chose = 3
                        flag = 0
                        flag_end = 1
