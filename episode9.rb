...
back2_img = Image.load("image/back/snow.jpg") #教室背景
...
Window.loop do
    ...
    # 背景の表示
    Window.draw_scale(0, 0, back2_img, 0.18, 0.18, 0, 0)

    # モノローグの部分はcase文で管理
    case flag2
    when 0
        Window.draw_scale(0, 0, back_img, 0.5, 0.5, 0, 0)
    ...
    when 1
        Window.draw_scale(0, 0, back_img, 0.5, 0.5, 0, 0)
        Window.draw_scale(19, 374, window_img, 0.5, 0.5, 0, 0)
        Window.draw_font(125, 374, "担任", font_name, color:[255, 255, 255])
        Window.draw_font(44, 404, "2学期も今日で終わりだが、ハメ外しすぎるなよー。\nはい、解散!", font, color:[165, 83, 126])
        if Input.mouse_push?(M_LBUTTON)
            flag2 = 2
        end
    when 2
    ...
    when 11
        ...
        if flag == 1 && flag_end == 0 then
            ...
            Window.draw_font(200, 279, "もっと豪華なのが良かったなぁ…", font, color:[165, 83, 126])
            # 選択肢3
            Window.draw_scale(109, 324, button_img, 0.6, 0.6, 0, 0)
            Window.draw_font(225, 329, "可愛いね。気に入ったよ。", font, color:[165, 83, 126])
            ...
            if timer >= 40
                ...
                if (109 < x && x <541 && 224 < y && y < 260) then # 選択肢1
                    ...
                    Window.draw_font(200, 279, "もっと豪華なのが良かったなぁ…", font, color:[255, 255, 255])
                    if Input.mouse_push?(M_LBUTTON) then
                        player_chose = 2
                        flag = 0
                        flag_end = 1
                    ...
                    Window.draw_font(225, 329, "可愛いね。気に入ったよ。", font, color:[255, 255, 255])
                    if Input.mouse_push?(M_LBUTTON) then
                        player_chose = 3
                        flag = 0
                        flag_end = 1
